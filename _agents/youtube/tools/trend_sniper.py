...
def main():
    ...
    if not sniper_data:
        print("💡 [Defensive Recovery] YouTube API Quota 초과 감지! B2B 리스크 진단/Paywall 트렌드용 모의 데이터로 대체하여 프로세스를 완주합니다.")
        mock_trends = {
            "유튜브 자동화": [
                "[유튜브 자동화] 채널: 테크비즈 | 제목: AI 자동화로 한 달에 $5,000 버는 시스템 구축 방법",
                "[유튜브 자동화] 채널: 노코드스타트업 | 제목: 1인 기업이 사외이사 없이 자동화로 연쇄 창업하는 법"
            ],
            "AI 비즈니스": [
                "[AI 비즈니스] 채널: 리스크프로 | 제목: EU AI Act 본격 발효, 당신의 SaaS가 불법이 되는 충격적 이유",
                "[AI 비즈니스] 채널: 비즈인사이트 | 제목: AI 생성 데이터의 법적 권한과 비식별화 실패 시 과징금 $L_{max}$ 폭탄 피하기"
            ],
            "마케팅 트렌드": [
                "[마케팅 트렌드] 채널: CRO해커 | 제목: 사용자를 패닉 상태로 만드는 B2B 결제 전환율(CVR) 극대화 심리학",
                "[마케팅 트렌드] 채널: 마케팅의 정석 | 제목: 공포 마케팅(Loss Aversion)과 한정판 심리 앵커를 활용한 무결성 세일즈 기법"
            ],
            "생산성 툴": [
                "[생산성 툴] 채널: 얼리어답터 | 제목: 업무 자동화 뒤에 숨겨진 보안 사각지대와 실시간 감사 로그의 중요성",
                "[생산성 툴] 채널: 테크리더 | 제목: DORA 규제 완벽 대비를 위한 제로 트러스트(Zero-Trust) 불변 감사 원장 시스템 도입법"
            ]
        }
        for q in chosen:
            if q in mock_trends:
                sniper_data.extend(mock_trends[q])
            else:
                sniper_data.append(f"[{q}] 채널: AI 트렌드스나이퍼 | 제목: {q} 마케팅 자동화의 비밀")
