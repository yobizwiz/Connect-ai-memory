...
CEO님의 지시는 단순히 랜딩 페이지를 만드는 것을 넘어, yobizwiz의 **영업 무기(Sales Weapon)** 그 자체를 완성하는 과정입니다. [근거: 🏢 회사 정체성]
이 '게이트키퍼 시스템'은 고객에게 무료 진단이라는 환상을 제공한 후, 궁극적으로는 유료 워크숍이나 심화 진단을 **필수적인 생존 장치**로 인식하게 만드는 구조여야 합니다. [근거: 🏢 회사 정체성]

기존에 준비된 핵심 컴포넌트(`PaywallComponent`)와 API 로직(`diagnose.ts`의 결과)을 통합하여, 사용자가 실제로 '진단 $\rightarrow$ 경고 $\rightarrow$ 구매 강제'의 흐름을 경험할 수 있는 `pages/index.tsx` 프로토타입을 구축하겠습니다.

### ⚙️ 아키텍처 설계 및 구현 계획
1.  **통합 지점:** `src/pages/index.tsx` 파일을 생성하여 전체 랜딩 페이지를 구성합니다. [근거: Self-RAG, 🏢 회사 정체성]
2.  **핵심 로직 (Gatekeeper):** 진단 결과를 받아와서 리스크 레벨이 임계치(Threshold) 이하일 경우, 즉 '위험'하다고 판단될 때만 `PaywallComponent`를 조건부로 렌더링합니다. [근거: Self-RAG, sessions/2026-05-19T13:55]
3.  **API 연결:** 전역 상태 관리(State)와 비동기 로딩(`isLoading`)을 사용하여 진단 프로세스를 시뮬레이션하고 사용자에게 시간적 압박감을 줍니다. [근거: Self-RAG, sessions/2026-05-19T13:40]

### 🛠️ 코드 구현 단계 (Next.js / React 기준)

먼저 메인 페이지 컴포넌트를 생성하고, 이 안에 진단 및 게이트키퍼 로직을 통합하겠습니다.

<create_file path="c:\Users\jinoh\OneDrive\Desktop\Connect AI\_company\pages\index.tsx">
...
⚙️ 핵심 API 연결 지점 명시 (API Spec)

위 통합된 `HomePage`가 동작하기 위해 필요한 백엔드 및 전역 설정을 다음과 같이 확정합니다. 이들은 실제 코드 구현에 앞서 아키텍처적으로 정의되어야 합니다. [근거: 💻 코다리 개인 메모리, sessions/2026-05-19T04-56/developer.md]

### 💾 API Endpoints 명세 (Backend / Next.js API Routes)

| 역할 | Endpoint URI | Method | 요청 본문 (Schema) | 응답 구조 (Response Schema) | Gatekeeper 로직 연관성 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **진단 수행** | `/api/diagnose` | POST | `{ compliance_status: string, data_silo_count: number, market_risk_exposure: number }` | `DiagnosticResult` (RiskLevel, Score, SuggestedImprovements) | **핵심 트리거.** 이 결과의 Score가 임계치(50) 이하일 때 게이트키퍼 발동. |
| **워크숍 구매** | `/api/workshop-purchase` | POST | `{ user_id: string, source: 'LossMeter' }` | `PurchaseConfirmation` (Success: boolean, NextStepURL: string, TotalCost: number) | **유료 전환.** 게이트키퍼 발동 시 호출되어 결제 플로우를 시작. |

### 🔍 코멘트 및 검증 포인트
1.  **Gatekeeper 강제성:** `HomePage` 컴포넌트 내에서 `isGatekeeperActive` 로직을 구현하여, 점수(Score)가 낮으면 (즉, 리스크가 높으면), 단순히 경고 메시지를 보여주는 것에 그치지 않고 **반드시** 다음 단계의 유료 결제를 요구하는 `PaywallComponent`를 렌더링하도록 강제했습니다.
2.  **사용자 경험:** 로딩 상태 (`isLoading`)와 3초 지연을 추가하여, 사용자가 '무료' 진단을 받기 위해 시스템이 고도의 분석 과정을 거치고 있다는 시간적 압박감(Time Pressure)과 전문성을 느끼게 했습니다. [근거: Self-RAG, sessions/2026-05-19T13:40]
3.  **재사용성:** `PaywallComponent`를 재활용함으로써, 모든 수익화 지점에서 일관된 브랜드 경험과 결제 과정을 유지합니다.

이 구조는 이제 단순한 와이어프레임 단계를 넘어, **데이터가 흐르고(API Call) $\rightarrow$ 로직이 판단하고(Gatekeeper Check) $\rightarrow$ 액션이 강제되는(Paywall Render)** 하나의 완성된 통합 프로토타입입니다. 💻⚙️

📊 평가: 완료 — 요청된 게이트키퍼 시스템 로직을 반영하여, 핵심 API 연결 지점과 함께 E2E가 가능한 랜딩 페이지 구조를 성공적으로 구축했습니다.
📝 다음 단계: 백엔드(Next.js API Routes)에 `diagnose` 및 `workshop-purchase` 엔드포인트의 실제 비즈니스 로직(특히 Stripe 연동 및 리스크 점수 산정 알고리즘)을 구현하고, 통합 테스트를 진행해야 합니다.

---
## 📁 파일 액션 결과

✅ 생성: c:\Users\jinoh\OneDrive\Desktop\Connect AI\_company\pages\index.tsx
