...
from src.services.loss_calculator_service import calculate_financial_loss_sync

router = APIRouter(prefix="/v1/risk", tags=["Risk Assessment"])

class RiskInput(BaseModel):
    """API 호출을 위한 표준화된 입력 스키마."""
    threat_score: float = Field(..., ge=0.0, le=1.0)  # 0.0 ~ 1.0 범위의 위협 점수 강제
...
async def calculate_financial_loss_api(input: RiskInput):
    ...
    try:
        ...
        if input.threat_score > 0.9:
            ...
            loss = calculate_financial_loss_sync(input.threat_score, input.user_context) * 1.2 # 보너스 손실액 가산
            return RiskOutput(
            ...
            loss = calculate_financial_loss_sync(input.threat_score, input.user_context) * 1.1
            return RiskOutput(
                estimated_loss_usd=round(loss, 2),
            ...
            loss = calculate_financial_loss_sync(input.threat_score, input.user_context) * 0.9
            return RiskOutput(
                estimated_loss_usd=round(loss, 2),
